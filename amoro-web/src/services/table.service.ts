...
export function getPartitionTable(
  ...
  const { catalog, db, table, filter, page, pageSize, sortBy, sortOrder, token } = params
  return request.get(`api/ams/v1/tables/catalogs/${catalog}/dbs/${db}/tables/${table}/partitions`, {
    params: { filter, page, pageSize, sortBy, sortOrder, token },
  })
}
...
export function getOptimizingProcesses(
  ...
  const { catalog, db, table, type, processCategory, status, page, pageSize, token } = params
